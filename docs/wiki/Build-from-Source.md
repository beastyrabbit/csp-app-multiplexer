...
available there. The `Protocol` and `Broker` libraries are plain `net10.0` and do build
...
The setup script follows the .NET 10 channel and installs its newest SDK in the
current user's profile. The latest C# language mode and `TreatWarningsAsErrors` are on; a
...
| Artifact | Size | Flags |
| --- | --- | --- |
| `…-needs-dotnet10.exe` | 0.9 MiB | `--self-contained false -p:PublishSingleFile=true` |
...
WPF solution and runs all tests. Each workflow installs the latest .NET 10 SDK
